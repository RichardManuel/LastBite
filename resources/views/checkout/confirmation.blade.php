    ...
    <div class="checkout-container">
        ...
        <div class="confirmation-content mt-4">
            ...
            <div class="order-details-item">
                ...
                <span class="eatery-name-text">{{ $restaurant->name }}</span>
            </div>
            <div class="order-details-item">
                <i class="bi bi-clock-history"></i> {{-- Atau bi-bag --}}
                <span class="item-name-text">{{ $pickup }}</span>
            </div>

            {{-- Link untuk melihat detail pesanan atau halaman order --}}
            @if (isset($order_id) && $order_id !== 'N/A' && $order_id !== 'DUMMY_' . substr($order_id, -5))
                <a href="{{ route('order', ['id' => $order_id]) }}" class="see-order-link">
                    <i class="bi bi-arrow-right"></i>
                    Press to see order
                </a>
            @else
                {{-- Tampilkan link ke halaman semua order user --}}
                <a href="{{ route('order') }}" class="see-order-link">
                    <i class="bi bi-arrow-right"></i>
                    See all your orders
                </a>
            @endif
