    ...
    <main class="container-fluid pt-4 pb-2">
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu shadow-lg" aria-labelledby="sortDropdown">
                        ...
                        <li><a class="dropdown-item py-2 px-3 {{ $selectedSort == 'default' ? 'active' : '' }}"
                                href="{{ url('/eatery?sort=most-popular&' . $filterQuery) }}#interactive-filters">Sort by
                                (Default)</a></li>
                        <li><a class="dropdown-item py-2 px-3 {{ $selectedSort == 'most-popular' ? 'active' : '' }}"
                                href="{{ url('/eatery?sort=best-rating&' . $filterQuery) }}#interactive-filters">Most
                                Popular</a></li>
                        <li><a class="dropdown-item py-2 px-3 {{ $selectedSort == 'best-rating' ? 'active' : '' }}"
                                href="?sort=best-rating">Best Rating</a></li>
                        <li><a class="dropdown-item py-2 px-3 {{ $selectedSort == 'newest' ? 'active' : '' }}"
                                href="{{ url('/eatery?sort=newest&' . $filterQuery) }}#interactive-filters">Newest</a></li>
                    </ul>
                </div>
            ...
            <div class="dropdown">
                ...
                <ul id="location-dropdown-menu" class="dropdown-menu dropdown-menu-end shadow-lg"
                    ...
                    <li><a href="{{ url('/eatery?' . $otherParams) }}#interactive-filters"
                            class="dropdown-item py-2 px-3 {{ !$selectedLocation ? 'active' : '' }}">All Locations</a></li>
                    <li>
        ...
        <div class="row row-cols-1 row-cols-xl- row-cols-lg-3 g-md-5">
            @foreach ($eateries as $eatery)
                <div class="col">
                    {{-- PERBAIKAN DI SINI: Gunakan array asosiatif untuk parameter route --}}
                    <a href="{{ route('eatery.detail', ['restaurant' => $eatery->id]) }}"
                        class="text-decoration-none d-block h-80">
                        <div class="card product-card d-flex flex-column h-100">
                            <div class="position-relative">
                                <img src="{{ asset('storage/' . $eatery->picture_of_restaurant) }}"
                                    alt="{{ $eatery->name }}" class="product-card-img">
                                <span
                                    class="position-absolute top-0 start-0 mt-3 ms-3 badge badge-custom-1 fw-bold shadow-sm">{{ $eatery->lunch_stock + $eatery->dinner_stock }}
                                    left</span>
                                @if ($eatery->isNew())
                                    <span
                                        class="position-absolute top-0 end-0 mt-3 me-3 badge badge-custom-2 fw-bold shadow-sm">New</span>
                                @endif
                            </div>
                            <div class="card-body d-flex flex-column p-2 p-md-4">
                                ...
                                <div class="mt-auto d-flex justify-content-between align-items-center pt-1">
                                    ...
                                    <span class="fw-bolder text-dark">Rp
                                        {{ number_format($eatery->pricing, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                    </a>
